...
const TONE_CONFIG = {
  Friendly: { color: "#16A34A", bg: "#DCFCE7", border: "#86EFAC", icon: "😊", label: "Friendly", desc: "Warm, polite — best for first reminder" },
  Firm: { color: "#D97706", bg: "#FEF9C3", border: "#FCD34D", icon: "📋", label: "Firm", desc: "Professional, direct — second reminder" },
  Urgent: { color: "#B91C1C", bg: "#FEE2E2", border: "#FCA5A5", icon: "🚨", label: "Urgent", desc: "Escalation warning — third reminder+" },
};
...
function generateMessage(invoice, tone) {
  ...
  const amt = `₹${Math.round(invoice.amount).toLocaleString("en-IN")}`;
  const due = fmtDate(invoice.due);

  if (tone === "Friendly") {
    return `Hi ${first},

Hope you're doing well! This is a gentle reminder that invoice ${invoice.id} for ${amt} was due on ${due}.
...
This is a follow-up regarding invoice ${invoice.id} for ${amt}, which remains unpaid as of ${due}.
...
Despite our previous reminders, invoice ${invoice.id} for ${amt} remains outstanding as of ${due}.
...
function SendConfirmModal({ invoice, tone, onClose, onConfirm, sending }) {
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(26,17,64,0.5)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ background: "#fff", borderRadius: 18, border: "1px solid #F0EAF8", width: "100%", maxWidth: 420, padding: 28, boxShadow: "0 20px 60px rgba(91,42,158,0.18)" }}>
        <div style={{ textAlign: "center", marginBottom: 20 }}>
          ...
          <div style={{ color: "#6B7894", fontSize: 13, marginTop: 4 }}>This will be sent to {invoice.client}</div>
        </div>
        <div style={{ background: "#F7F9FC", borderRadius: 10, padding: "12px 14px", marginBottom: 20 }}>
          {[
            ...
            { label: "Amount", value: `₹${Math.round(invoice.amount).toLocaleString("en-IN")}` },
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={onClose} disabled={sending} style={{ flex: 1, padding: "11px", borderRadius: 10, border: "1.5px solid #E2E8F4", background: "#fff", color: "#6B7894", cursor: "pointer", fontFamily: "'Space Grotesk',sans-serif", fontWeight: 600, fontSize: 14 }}>Cancel</button>
          <button onClick={onConfirm} disabled={sending} style={{ flex: 1, padding: "11px", borderRadius: 10, border: "none", background: "linear-gradient(120deg,#FF6B81,#FF9472)", color: "#fff", cursor: "pointer", fontFamily: "'Space Grotesk',sans-serif", fontWeight: 700, fontSize: 14, boxShadow: "0 4px 14px rgba(255,107,129,0.3)" }}>
...
export default function DeadlineMessagesPage() {
    ...
    async function load() {
      ...
      try {
        ...
        if (mapped.length > 0) {
          ...
          setTone(getSuggestedTone(mapped[0].remindersSent));
        }
      } catch (err) {
        if (!cancelled) setError(err.message || "Failed to load pending invoices");
      } finally {
  ...
  if (loading) {
    return (
      <div style={{ textAlign: "center", padding: "80px 0", color: "#9AA7C2" }}>
        <p style={{ fontSize: 14 }}>Loading pending invoices...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div style={{ textAlign: "center", padding: "80px 0" }}>
        <p style={{ fontSize: 14, color: "#DC2626", fontWeight: 600 }}>Couldn't load reminders</p>
    ...
    return (
      <>
        <div style={{ marginBottom: 24 }}>
          <h1 style={{ fontFamily: "'Space Grotesk',sans-serif", fontWeight: 700, fontSize: 24, color: "#1A1140", margin: 0 }}>Deadline Messages</h1>
          <p style={{ fontSize: 13.5, color: "#6B7894", margin: "4px 0 0" }}>Automatic reminders before and after due dates</p>
        </div>
        <div style={{ textAlign: "center", padding: "64px 0", color: "#9AA7C2" }}>
          ...
          <p style={{ fontSize: 13, margin: 0 }}>All caught up — nothing pending right now.</p>
        </div>
      </>
    );
  }
  ...
  const handleConfirmSend = async () => {
    setSending(true);
    try {
      const updated = await api.sendReminder(selected.rawId, tone, message);
      setInvoiceList(prev => prev.map(inv =>
        inv.id === selected.id
          ? { ...inv, remindersSent: updated.remindersSent, history: updated.history }
          : inv
      ));
      setTone(getSuggestedTone(updated.remindersSent));
  ...
  const stats = [
    ...
    { label: "Pending Reminders", value: invoiceList.length, icon: "⏳", color: "#D97706" },
    { label: "Overdue Invoices", value: invoiceList.filter(i => i.status === "Overdue").length, icon: "🚨", color: "#DC2626" },
    { label: "Never Reminded", value: invoiceList.filter(i => i.remindersSent === 0).length, icon: "🆕", color: "#0EA5E9" },
  ];
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        <div>
          ...
          <p style={{ fontSize: 13.5, color: "#6B7894", margin: "4px 0 0" }}>
            Pick the right tone for each reminder — every message feels human, not automated.
          </p>
        </div>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <div>
                        ...
                        <div style={{ color: "#6B7894", fontSize: 12.5, marginTop: 2 }}>{inv.client} · Due {fmtDate(inv.due)}</div>
                      </div>
                    </div>
                    <div style={{ textAlign: "right", flexShrink: 0 }}>
                      <div style={{ fontFamily: "'Space Grotesk',sans-serif", fontWeight: 700, fontSize: 15, color: "#1A1140" }}>₹{Math.round(inv.amount).toLocaleString("en-IN")}</div>
                      <div style={{ fontSize: 11, color: "#9AA7C2", marginTop: 2 }}>{inv.remindersSent} sent</div>
                    </div>
                  </div>

                  <div style={{ marginTop: 10, display: "flex", alignItems: "center", gap: 6, background: "#F7F9FC", borderRadius: 8, padding: "7px 10px" }}>
                    <span style={{ fontSize: 13 }}>🤖</span>
                    <span style={{ fontSize: 12, color: "#6B7894" }}>Suggested:</span>
                    ...
                    <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", borderBottom: i < selected.history.length - 1 ? "1px solid #F4F0FC" : "none" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: 12, color: "#9AA7C2", marginTop: 2 }}>Sent {fmtDate(h.sent)}</div>
                        </div>
                      </div>
                    </div>
                  );
          ...
          <div style={{ background: "#fff", borderRadius: 16, padding: 22, boxShadow: "0 2px 12px rgba(91,42,158,0.08)", border: "1px solid #F0EAF8", position: "sticky", top: 20 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18, paddingBottom: 16, borderBottom: "1px solid #F0EAF8" }}>
              ...
              <div style={{ textAlign: "right" }}>
                <div style={{ fontFamily: "'Space Grotesk',sans-serif", fontWeight: 800, fontSize: 18, color: "#1A1140" }}>₹{Math.round(selected.amount).toLocaleString("en-IN")}</div>
                <div style={{ fontSize: 11.5, color: "#9AA7C2" }}>Due {fmtDate(selected.due)}</div>
              </div>
            </div>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        ...
                        <div>
                          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                            ...
                            {isSuggested && (
                              <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 7px", borderRadius: 10, background: "#EDE9FE", color: "#6D28D9" }}>Suggested</span>
                            )}
                          </div>
            ...
            <div style={{ textAlign: "center", marginTop: 10, fontSize: 12, color: "#9AA7C2" }}>
              {selected.remindersSent} reminder{selected.remindersSent !== 1 ? "s" : ""} sent so far
            </div>
          </div>
        </div>
      </div>
